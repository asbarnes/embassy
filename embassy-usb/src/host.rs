...
impl ConfigurationDescriptor {
    ...
    pub fn parse_interface<'a>(&'a self, index: usize) -> Option<InterfaceDescriptor<'a>> {
        ...
        let interface_bytes =
            if let Some((offset, _)) = Self::identify_interface(next_interface_buffer) {
...
impl<D: USBHostDriverTrait> UsbHost<D> {
    ...
    async fn device_set_address(&mut self, addr: u8) -> Result<(), ()> {
        ...
        self.driver
            .control_request_out(packet.as_bytes(), &[])
            .await?;
    ...
    pub async fn device_request(
        ...
        let _ = self
            .driver
            .control_request_in(packet, &mut buf[..req_length as usize])
            .await;
    ...
    pub async fn device_response(
        ...
        let _ = self
            .driver
            .control_request_out(packet, &buf)
            .await;
    ...
    pub async fn device_request_descriptor<T: USBDescriptor, const SIZE: usize>(
        &mut self,
    ) -> Result<T, ()> {
        ...
        let _ = self
            .driver
            .control_request_in(packet.as_bytes(), &mut buf)
            .await;
    ...
    pub async fn device_request_descriptor_bytes<T: USBDescriptor>(
        &mut self,
        bytes: &mut [u8],
    ) -> Result<usize, ()> {
        // The wValue field specifies the descriptor type in the high byte
        ...
        self.driver
            .control_request_in(packet.as_bytes(), bytes)
            .await
    }
    ...
    pub async fn enumerate(
        &mut self,
    ) -> Result<(DeviceDescriptor, ConfigurationDescriptor), EnumerationError> {
        ...
        trace!("Full Device Descriptor: {:?}", device_descriptor);

        let configuration_descriptor = self
        ...
        let total_len = configuration_descriptor.total_len as usize;
